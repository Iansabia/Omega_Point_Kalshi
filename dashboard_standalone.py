...
def Page():
    ...
    def create_model():
        ...
        new_model = PredictionMarketModel(
    ...
    with solara.Column(style={"padding": "20px"}):
        ...
        with solara.Card("Controls", elevation=2):
            with solara.Column():
                ...
                # Status
                if model.value is not None:
                    solara.Markdown(f"**Step:** {current_step.value} | **Agents:** {len(list(model.value.agents))} | **Price:** ${model.value.current_price:.3f}")
...
def MarketCharts(model):
    ...
    # Price chart
    fig.add_trace(
        go.Scatter(x=df.index, y=df['market_price'], mode='lines', name='Market Price',
                   line=dict(color='blue', width=2)),
        row=1, col=1
    )
    fig.add_trace(
        go.Scatter(x=df.index, y=df['fundamental_value'], mode='lines', name='Fundamental',
    ...
    # Volume chart
    fig.add_trace(
        go.Bar(x=df.index, y=df['total_volume'], name='Volume', marker_color='green'),
        row=1, col=2
    )

    # Spread chart
    fig.add_trace(
        go.Scatter(x=df.index, y=df['bid_ask_spread'], mode='lines', name='Spread',
    ...
    if len(df) > 1:
        ...
        fig.add_trace(
            go.Scatter(x=df.index, y=cum_returns, mode='lines', name='Returns',
...
def AgentCharts(model):
    ...
    fig = make_subplots(
        rows=1, cols=2,
        subplot_titles=('Wealth Distribution', 'Wealth by Agent Type')
    )

    # Wealth histogram
    fig.add_trace(
        go.Histogram(x=latest_data['wealth'], nbinsx=30, name='Wealth',
                     marker_color='blue'),
        row=1, col=1
    )
    ...
    for agent_type in latest_data['agent_type'].unique():
        ...
        fig.add_trace(
            go.Box(y=type_data['wealth'], name=agent_type),
            row=1, col=2
        )
