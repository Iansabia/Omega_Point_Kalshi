...
def MarketPriceChart(model: PredictionMarketModel):
    ...
    # Volume chart
    fig.add_trace(go.Bar(x=model_data.index, y=model_data["total_volume"], name="Volume", marker_color="green"), row=2, col=1)
    ...
    fig.update_layout(height=800, showlegend=True, title_text="Market Overview", title_x=0.5)
...
def AgentBehaviorChart(model: PredictionMarketModel):
    ...
    fig = make_subplots(
        ...
        specs=[[{"type": "box"}, {"type": "box"}], [{"type": "histogram"}, {"type": "bar"}]],
    )
    ...
    for agent_type in latest_data["agent_type"].unique():
        ...
        fig.add_trace(go.Box(y=type_data["wealth"], name=agent_type, boxmean="sd"), row=1, col=1)

    # Position by agent type (box plot)
    for agent_type in latest_data["agent_type"].unique():
        type_data = latest_data[latest_data["agent_type"] == agent_type]
        fig.add_trace(go.Box(y=type_data["position"], name=agent_type, showlegend=False), row=1, col=2)

    # Wealth histogram
    fig.add_trace(
        go.Histogram(x=latest_data["wealth"], nbinsx=30, name="Wealth Distribution", marker_color="blue"), row=2, col=1
    )
    ...
    fig.update_layout(height=800, showlegend=True, title_text="Agent Behavior Analysis", title_x=0.5)
...
def PerformanceChart(model: PredictionMarketModel):
    ...
    rolling_sharpe = (
        pd.Series(returns).rolling(window=20).apply(lambda x: np.sqrt(252) * x.mean() / x.std() if x.std() > 0 else 0)
    )
    ...
    # Rolling Sharpe
    fig.add_trace(
        go.Scatter(x=model_data.index, y=rolling_sharpe, mode="lines", name="Sharpe Ratio", line=dict(color="blue", width=2)),
        row=2,
        col=1,
    )
    ...
    fig.add_trace(
        go.Scatter(
            x=model_data.index, y=drawdown, mode="lines", name="Drawdown", line=dict(color="red", width=2), fill="tozeroy"
        ),
        row=3,
        col=1,
    )
...
def OrderBookChart(model: PredictionMarketModel):
    ...
    # Depth chart (bids)
    fig.add_trace(go.Bar(x=bid_prices, y=bid_quantities, name="Bids", marker_color="green", orientation="v"), row=1, col=1)

    # Depth chart (asks)
    fig.add_trace(go.Bar(x=ask_prices, y=ask_quantities, name="Asks", marker_color="red", orientation="v"), row=1, col=1)

    # Add mid price line
    mid_price = order_book.get_mid_price()
    if mid_price:
        fig.add_vline(x=mid_price, line_dash="dash", line_color="blue", annotation_text=f"Mid: {mid_price:.3f}", row=1, col=1)
...
model_params = {
    "n_noise_traders": {"type": "SliderInt", "value": 100, "label": "Noise Traders", "min": 10, "max": 500, "step": 10},
    "n_informed_traders": {"type": "SliderInt", "value": 20, "label": "Informed Traders", "min": 5, "max": 100, "step": 5},
    "n_arbitrageurs": {"type": "SliderInt", "value": 10, "label": "Arbitrageurs", "min": 1, "max": 50, "step": 5},
    "n_market_makers": {"type": "SliderInt", "value": 5, "label": "Market Makers", "min": 1, "max": 20, "step": 1},
...
def create_model_from_params(params):
    ...
    if params.get("n_noise_traders", 0) > 0:
        agent_config["noise_trader"] = {"count": params["n_noise_traders"], "wealth": 1000}

    if params.get("n_informed_traders", 0) > 0:
        agent_config["informed_trader"] = {"count": params["n_informed_traders"], "wealth": 10000, "information_quality": 0.8}

    if params.get("n_arbitrageurs", 0) > 0:
        agent_config["arbitrageur"] = {"count": params["n_arbitrageurs"], "wealth": 50000, "detection_speed": 0.9}

    if params.get("n_market_makers", 0) > 0:
        agent_config["market_maker"] = {"count": params["n_market_makers"], "wealth": 100000, "risk_param": 0.1}

    market_config = {"initial_price": params.get("initial_price", 0.5), "tick_size": 0.01}
...
page = SolaraViz(
    ...
    components=[MarketPriceChart, AgentBehaviorChart, PerformanceChart, OrderBookChart],
