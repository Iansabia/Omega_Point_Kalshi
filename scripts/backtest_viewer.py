...
def BacktestResultsView():
    ...
    with solara.Column():
        ...
        # Charts
        fig = make_subplots(
            rows=2, cols=2, subplot_titles=("Equity Curve", "Per-Game P&L Distribution", "Drawdown", "Cumulative Return")
        )

        # Equity curve
        fig.add_trace(
            go.Scatter(x=df["game"], y=df["capital"], mode="lines", name="Capital", line=dict(color="blue", width=2)),
            row=1,
            col=1,
        )

        # P&L distribution
        fig.add_trace(go.Histogram(x=df["pnl"], nbinsx=30, name="P&L", marker_color="green"), row=1, col=2)
        ...
        if len(df) > 0:
            ...
            fig.add_trace(
                go.Scatter(
                    x=df["game"], y=drawdown, mode="lines", name="Drawdown", line=dict(color="red", width=2), fill="tozeroy"
                ),
                row=2,
                col=1,
            )
        ...
        if len(df) > 0 and df["capital"].iloc[0] != 0:
            ...
            fig.add_trace(
                go.Scatter(x=df["game"], y=cum_return, mode="lines", name="Return", line=dict(color="purple", width=2)),
                row=2,
                col=2,
            )
...
def MonteCarloView():
    ...
    def run_monte_carlo():
        ...
        for _ in range(n_sims.value):
            ...
            simulations.append({"final_capital": capital, "return": (capital / 10000 - 1) * 100, "equity_curve": equity_curve})
    ...
    with solara.Column():
        ...
        if results.value is not None:
            ...
            # Return distribution
            fig.add_trace(go.Histogram(x=returns, nbinsx=50, name="Returns", marker_color="blue"), row=1, col=1)
            ...
            fig.add_trace(
                go.Scatter(x=sorted_returns, y=cumprob, mode="lines", name="Cumulative", line=dict(color="purple", width=2)),
                row=2,
                col=1,
            )
            ...
            prob_loss = [len([r for r in returns if r <= level]) / len(returns) * 100 for level in loss_levels]
            fig.add_trace(
                go.Scatter(
                    x=loss_levels, y=prob_loss, mode="lines", name="Risk", line=dict(color="red", width=2), fill="tozeroy"
                ),
                row=2,
                col=2,
            )
...
def TradeAnalysisView():
    ...
    with solara.Column():
        ...
        # Charts
        fig = make_subplots(
            rows=2, cols=2, subplot_titles=("Price Distribution", "Quantity Distribution", "Trades Over Time", "Buy vs Sell")
        )

        # Price distribution
        fig.add_trace(go.Histogram(x=df["price"], nbinsx=50, name="Price"), row=1, col=1)

        # Quantity distribution
        fig.add_trace(go.Histogram(x=df["quantity"], nbinsx=50, name="Quantity"), row=1, col=2)

        # Trades over time
        trades_per_game = df.groupby("game_id").size()
        fig.add_trace(go.Scatter(x=trades_per_game.index, y=trades_per_game.values, mode="lines", name="Trades"), row=2, col=1)

        # Buy vs Sell
        side_counts = df["side"].value_counts()
        fig.add_trace(go.Bar(x=side_counts.index, y=side_counts.values, name="Side"), row=2, col=2)
