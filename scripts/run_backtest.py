...
class HistoricalBacktest:
    ...
    def generate_historical_scenarios(self):
        ...
        if self.use_real_data and self.data_fetcher:
            ...
            scenarios = self.data_fetcher.load_historical_backtest_data(season=self.season, max_games=self.n_games)
    ...
    def run_backtest(self):
        ...
        for i, scenario in enumerate(scenarios):
            ...
            self.equity_curve.append(
                {"game": i + 1, "capital": capital, "pnl": game_pnl, "return": game_pnl / self.initial_capital}
            )
    ...
    def _simulate_game(self, scenario):
        ...
        # Create market model
        market_config = {"initial_price": scenario["initial_price"], "tick_size": 0.01}

        model = PredictionMarketModel(
            agent_config=self.agent_config, config={"market": market_config}, seed=scenario["game_id"]
        )
    ...
    def _calculate_game_pnl(self, trades, outcome):
        ...
        for trade in trades:
            ...
            # Transaction cost using Kalshi's real fee formula (July 2025+)
            # Taker fee: 0.07 × C × P × (1-P)
            # Using taker fee as conservative estimate (market orders)
            cost = 0.07 * quantity * price * (1 - price)
